...
img = cv2.imread("modifiche\\bradley_cooper.jpg")
img2 = cv2.imread("modifiche\\volto.jpg")
face_cascade = cv2.CascadeClassifier('modifiche\\haarcascade_frontalface_alt.xml')
...
predictor = dlib.shape_predictor("modifiche\\shape_predictor_68_face_landmarks.dat")
# Face 1
...
while(True):
    ...
    for(x,y,w,h) in faces:
        #print(x,y,w,h)
        padding_top = int(h * 0.50)
        padding_bottom = int(h * 0.35)
        start_cord_x = x
        start_cord_y = y - padding_top
        end_cord_x = x + w
        new_h = h + padding_top + padding_bottom
        end_cord_y = start_cord_y + new_h
        faccia = gray[start_cord_y:end_cord_y, start_cord_x:end_cord_x]
        roi_gray = gray[y:y+h, x:x+w] #(y_cordi_start, y_cordi_emd)

        roi_color = frame[y:y+h, x:x+w]
        #volto_sconosciuto_features = face_recognition.face_encodings(frame, roi_color)
